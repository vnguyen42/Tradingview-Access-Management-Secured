...
class tradingview:
    ...
    def __init__(self):
        ...
        # Try to read the sessionid from a local file
        try:
        ...
        test = requests.get(config.urls["tvcoins"], headers=headers)
        print(test.text)
        print('sessionid from file: ' + self.sessionid)
        if test.status_code != 200:
            print('Session ID from file is invalid')
            ...
            userAgent = 'TWAPI/3.0 ({}; {}; {})'.format(platform.system(),
                                                        platform.version(),
                                                        platform.release())
            print(userAgent)
            login_headers = {
            ...
            # Save the new sessionid to a file
    ...
    def add_access(self, access_details, extension_type, extension_length):
        ...
        if not noExpiration:
            ...
            access_details['status'] = 'Success' if (
                add_access_response.status_code in [200, 201]) else 'Failure'
        return access_details
    ...
    def remove_access(self, access_details):
        ...
        remove_access_response = requests.post(config.urls['remove_access'],
                                               ...
                                               headers=headers)
        access_details['status'] = 'Success' if (
            remove_access_response.status_code == 200) else 'Failure'
